...
@section('content')
    ...
    <section class="py-20 bg-slate-50 border-y border-slate-100">
        <div class="max-w-7xl mx-auto px-6 text-center">
            <h2 class="text-2xl font-extrabold text-slate-800 mb-2">Official Partners</h2>
            <p class="text-slate-500 mb-12 font-medium">Bekerja sama dengan brand dan organisasi global untuk pengalaman terbaik.</p>
            
            <div class="flex flex-wrap justify-center gap-12 items-center opacity-60">
                @forelse($partners as $partner)
                    <a href="{{ $partner->link ?? '#' }}" target="_blank" class="hover:opacity-100 transition duration-300" title="{{ $partner->name }}">
                        <img src="{{ asset('storage/' . $partner->logo) }}" 
                             alt="{{ $partner->name }}" 
                             class="h-12 w-auto grayscale hover:grayscale-0 transition-all object-contain">
                    </a>
                @empty
                    <p class="text-slate-400 italic text-sm">Belum ada partner resmi terdaftar.</p>
                @endforelse
            </div>
        </div>
    </section>
